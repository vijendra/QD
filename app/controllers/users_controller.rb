class UsersController < ApplicationController
  ...
  def activate
    ...
    when (!params[:activation_code].blank?) && user && !user.active?
      ...
      user.update_attribute('type', 'Dealer') #This is to make user as of type dealer. needs to be changed if other type users signup in future
      flash[:notice] = "Signup complete! Please sign in to continue."
      redirect_to login_path
    when params[:activation_code].blank?
      flash[:error] = "The activation code was missing.  Please follow the URL from your email."
      user.profile = Profile.create
      user.update_attribute('type', 'Dealer') #This is to make user as of type dealer.
