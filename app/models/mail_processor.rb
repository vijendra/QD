class MailProcessor < ActionMailer::Base
  ...
  def receive(mail)
    ...
    if (mail.from.to_s =~ /@tranzactis.com/ || mail.subject =~ /Order Fulfillment Notification/)  
      ...
      dealer_profile = Profile.find_by_name(dealer)
      balance = dealer_profile.current_balance
      trigger = TriggerDetail.create(:dealer_id => dealer, :data_source => 'marketernet', :total_records => no_of_records, :order_number => order_number )
      data_source2 = ['listid', 'lname', 'fname', 'mname', 'address', 'address2', 'city', 'state', 'zip',  'zip4', 'level', '', 'auto17', 'crrt', 'dpc', 'phone_num']
      FasterCSV.foreach(orders_csv, :headers => :false) do |row|
        balance = balance -1
        data_set = {:dealer_id => dealer_id, :trigger_detail_id => trigger.id}
        data_source2.map {|f| data_set[f] = row[data_source2.index(f)]}
        QdProfile.create(data_set)
      end
      dealer_profile.update_attribute('current_balance', balance) 
    end 
