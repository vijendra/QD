...
for data in @profiles
 ...
 p_pdf.bounding_box([box.right - 225, box.top - 110], :width => 200) do
  ...
  p_pdf.bounding_box([box.left + 115, box.top - 125], :width => 200) do
  ...
  p_pdf.text "Dear #{h(@name)}", :at => [box.left + 50, box.top - 255]

  p_pdf.text_box "#{h(@first_para)}",
    :width    =>  box.right - 70, :height => 100,
    :at       => [box.left + 50, box.top - 260]

  p_pdf.text_box "#{h(@name)}, #{h(@sec_para)}",
    :width    =>  box.right - 70, :height => 100,
    :at       => [box.left + 50, box.top - 335]

  p_pdf.bounding_box([box.left + 50, box.top - 400], :width => box.right - 60) do
